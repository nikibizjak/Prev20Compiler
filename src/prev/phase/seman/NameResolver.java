...
public class NameResolver extends AstFullVisitor<Object, NameResolver.Mode> {
	...
	public Object visit(AstTrees<?> trees, Mode arg) {
		...
		// Iterate over everything else, statements, components, arguments, ...
		for (AstTree tree : trees) {
			// Skip declarations that have already been done in the previous 6 loops
			if (tree instanceof AstTypeDecl || tree instanceof AstVarDecl || tree instanceof AstFunDecl)
				continue;
			// Use Mode.BODY, it does not matter
